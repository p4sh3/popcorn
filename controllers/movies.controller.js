...
export class MovieController {
  ...
  static async getAll(req, res) {
    ...
    if (!country || !TLDs[country]) throw new AppError("Invalid or missing country parameter", 400)

    const { security } = await this.#getParams({ country });
    const url = `https://cinepolis${TLDs[country]}/wp-admin/admin-ajax.php?action=get_sites_data` +
        `&security=${security}` +
        `&single_cinema=0&single_movie=0&isPopup=false`;

    const { movies } = await this.#fetchData({url});
    if (!movies?.length) throw new AppError(`No movies found for ${COUNTRIES[country]}`, 404)
